...
let inputFiltered = input.filter(
    str => {
        ...
        const ranges = data[0];
        const letter = data[1].split(":")[0];
        const password = data[2];

        const min = parseInt(ranges.split("-")[0]);
        const max = parseInt(ranges.split("-")[1]);

        let counter = password.split("").filter(eachLetter => {
        ...
        return min <= counter.length && counter.length <= max;
    }
);

console.log("La respuesta es:");
console.log(inputFiltered.length);

//part 2
inputFiltered = input.filter(
    str => {
        const data = str.split(" ");
        const ranges = data[0];
        const letter = data[1].split(":")[0];
        ...
        let flag = false;
        password.split("").forEach((eachLetter, index) => {
            if(index === (min-1) && eachLetter === letter) {
                flag = !flag;
            } else if(index === (max-1) && eachLetter === letter) {
                flag = !flag;
            }
        });
        return flag;
